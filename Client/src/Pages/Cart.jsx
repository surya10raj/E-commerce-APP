    ...
    <div className="container mx-auto">
      ...
      {cartItems.map((item) => (
        <div key={item._id} className="flex items-center bg-gray-100 p-4 rounded-lg shadow-md">
          ...
          <button onClick={() => removeItem(item._id)} className="ml-auto bg-red-500 text-white px-4 py-2 rounded-md">
            Remove
          </button>
        </div>
      ))}
      <div className="text-center mt-8">
        ...
        <button className="mt-4 bg-green-500 text-white px-4 py-2 rounded-md">Proceed to Buy</button>
      </div>
    </div>
  );
}
